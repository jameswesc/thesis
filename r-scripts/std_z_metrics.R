...
process_site <- function(site) {
    site_dir <- tidyverse::str_c(base_dir, "/", site)
    lidar_file <- tidyverse::str_c(site_dir, "/", site, "_MGA2020.copc.laz")
    plots_file <- tidyverse::str_c(site_dir, "/", site, "_plots_MGA2020.geojson")
...
write_out_metrics <- function(metrics, site) {
    ...
    write_json(
        ...
        tidyverse::str_c(site_dir, "/", site, "_std_z_metrics.json")
    )
    write_json(
        st_drop_geometry(metrics$std_z_metrics_nolownoise),
        tidyverse::str_c(site_dir, "/", site, "_std_z_metrics_nolownoise.json")
    )
}
